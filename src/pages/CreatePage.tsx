...
import { Save, AlertCircle, Plus, Trash2, ArrowLeft, Eye, X, LayoutTemplate, GripVertical, Copy } from "lucide-react";
                    ...
                    <div className="p-5 rounded-lg bg-card border border-border space-y-4">
                      ...
                      {form.optionalBlocks.map((block, i) => (
                        <div key={i} className="p-4 rounded-lg bg-secondary space-y-2">
                          ...
                          <RichTextEditor value={block.content} onChange={(val) => updateOptionalBlock(i, "content", val)} placeholder="Conteúdo do bloco customizado..." />
                          <div className="grid grid-cols-2 gap-2 mt-2">
                            ...
                            <input value={block.linkLabel || ""} onChange={(e) => updateOptionalBlock(i, "linkLabel", e.target.value)} className="px-3 py-2 rounded-lg border border-border bg-background text-sm text-foreground" placeholder="Texto do link (opcional)" />
                          </div>
                          <div className="mt-2">
